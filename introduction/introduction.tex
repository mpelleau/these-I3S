...
\section{Organisation}\label{sec:orga}

Pour compiler ce document, vous pouvez utiliser le \mytt{Makefile}. Sinon, on m'a dit dans l'oreillette qu'il existait des éditeurs tels que Texmaker qui gèrent eux-mêmes la compilation.
...
Le résumé et les mots-clés en français et en anglais se trouvent dans le fichier \mytt{resume.tex}.
...
Il existe plusieurs façons de créer des chapitres :
\begin{itemize}
  \item \verb|\chapter{Introduction}| crée un chapitre numéroté ;
  \item \verb|\chapter*{Titre}| crée un chapitre sans numérotation qui n'apparaitra pas dans le sommaire ;
  \item \verb|\Chapter{Notations}| crée un chapitre sans numérotation qui apparaitra dans le sommaire.
\end{itemize}

\begin{remark}
  La numérotation des sections et théorèmes dépend de celle du chapitre. Un chapitre sans numérotation ne devrait pas contenir de sections numérotées ni de théorèmes, sinon il risque d'y avoir des problèmes de numérotation.
\end{remark}

\begin{remark}
  Par défaut, une section est numérotée ainsi :\\\verb|\renewcommand\thesection{\thechapter.\arabic{section}}|\\La numérotation des théorèmes dépend des sections et donc dépend des chapitres comme celles-ci. Ce système permet d'avoir des numéros de sections et de théorèmes uniques dans tout le document. En effet, il n'existe qu'une seule section \ref{sec:orga}, c'est la première section du premier chapitre. Si vous voulez changer le système de numérotation pour que le chapitre n'apparaisse plus, vous le pouvez avec la commande :\\\verb|\renewcommand\thesection{\arabic{section}}|\\Dans ce cas, la numérotation ne sera plus unique. Il faudra donc préciser dans quel chapitre se trouve la section à laquelle vous faites référence (la section 1 du chapitre 1). De même, les théorèmes n'auront plus de numéro unique, il faudra préciser le chapitre dans lequel il se trouve (la définition 2.1 du chapitre 2). Et donc, dans la liste des définitions, il pourra y avoir plusieurs définitions avec le même numéro, ce qui ne semble pas être une bonne pratique.
\end{remark}

Au début de chaque chapitre, vous pouvez inclure différents éléments à l'aide des commandes suivantes :

\begin{itemize}
  \item \verb|\chapres{1er résumé}{2e résumé}| ajoute les 2 résumés passés en paramètres suivis de la table des matières, cette commande est utilisée pour le chapitre \ref{chap:patisserie} ;
  \item \verb|\chapintro{un seul résumé}| ajoute le résumé passé en paramètre suivi de la table des matières, cette commande est utilisée pour le chapitre \ref{chap:macarons} ;
  \item \verb|\chaptoc| ajoute la table des matières, cette commande est utilisée pour le chapitre \ref{chap:chap4}.
\end{itemize}
...
Le document est désormais compilé pour être compatible avec la norme PDF/A-3B.
...
Si cela ne vous convient pas, vous pouvez commenter ou modifier les lignes 89 et 90 du fichier \mytt{mesmacros.sty} :
...
Si vous souhaitez que votre publication apparaisse dans la bibliographie générale à la fin du manuscrit vous pouvez soit la citer avec \mytt{\textbackslash cite} ou l'insérer avec \mytt{\textbackslash nocite}. \nocite{DHHS2011}

\begin{remark}
  Si vous utilisez un style de bibliographie numérotant les références, vous aurez des problèmes de numérotation si une publication est à la fois citée en utilisant la macro \mytt{\textbackslash cite} et la macro \mytt{\textbackslash citemine}.
\end{remark}
