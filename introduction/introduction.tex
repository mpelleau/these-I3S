...
Cette classe \LaTeX est basée sur la classe \mytt{these-LINA} écrite par Frédéric Goualard. La classe a été modifiée pour correspondre au modèle de thèse demandé par l'ED STIC de l'Université Côte d'Azur.
...
\section{Organisation}
  ...
  Pour compiler ce document vous pouvez utiliser le \mytt{Makefile}.
  ...
  Ce n'est pas nécessaire mais c'est plus facile pour s'y retrouver après, et dans chaque dossier il y a le fichier principal, et il peut y avoir d'autre fichiers ou dossiers (par exemple un dossier \mytt{img/}).
